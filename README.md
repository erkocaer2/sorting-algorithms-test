...
*   **`sorting_algorithms.py`**: The main module with basic sorting algorithms (like Bubble Sort).
...
*   **`tests/test_sorting.py`**: Contains unit tests using the `unittest` module. It checks if the algorithms give the correct results.
...
Because of the workflow in the `.github/workflows/python-tests.yml` file, when you `push` or make a `pull_request` to the `main` branch:
